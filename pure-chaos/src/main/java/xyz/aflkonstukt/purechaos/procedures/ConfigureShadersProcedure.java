	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity) {
		...
		if (world.isClientSide()) {
			if (!(Minecraft.getInstance().gameRenderer.currentEffect() != null)) {
				...
				if ((Minecraft.getInstance().gameRenderer.currentEffect().toString() == null ? "" : Minecraft.getInstance().gameRenderer.currentEffect().toString()).equals("purechaos:shaders/meth.json")
						&& !(entity instanceof LivingEntity _livEnt16 && _livEnt16.hasEffect(PurechaosModMobEffects.HIGH_EFFECT))) {
					Minecraft.getInstance().gameRenderer.shutdownEffect();
				} else if ((Minecraft.getInstance().gameRenderer.currentEffect().toString() == null ? "" : Minecraft.getInstance().gameRenderer.currentEffect().toString()).equals("purechaos:shaders/minecraft/desaturate.json")
						&& !entity.getData(PurechaosModVariables.PLAYER_VARIABLES).having_nightmare) {
					Minecraft.getInstance().gameRenderer.shutdownEffect();
					if (entity instanceof LivingEntity _entity)
						_entity.removeAllEffects();
				} else if ((Minecraft.getInstance().gameRenderer.currentEffect().toString() == null ? "" : Minecraft.getInstance().gameRenderer.currentEffect().toString()).equals("purechaos:shaders/minecraft/ntsc.json")
						&& !((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, ResourceLocation.parse("purechaos:backrooms_dimension")))) {
					Minecraft.getInstance().gameRenderer.shutdownEffect();
				} else if ((Minecraft.getInstance().gameRenderer.currentEffect().toString() == null ? "" : Minecraft.getInstance().gameRenderer.currentEffect().toString()).equals("purechaos:shaders/minecraft/art.json")
						&& !(entity instanceof LivingEntity _livEnt27 && _livEnt27.hasEffect(PurechaosModMobEffects.DRUNK))) {
					Minecraft.getInstance().gameRenderer.shutdownEffect();
				} else if ((Minecraft.getInstance().gameRenderer.currentEffect().toString() == null ? "" : Minecraft.getInstance().gameRenderer.currentEffect().toString()).equals("purechaos:shaders/pale_shader.json")
