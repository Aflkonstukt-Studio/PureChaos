...
public class KondomnepitajzastoovoradimalispoileralertonimarupuProjectileEntity extends AbstractArrow implements ItemSupplier {
	...
	public KondomnepitajzastoovoradimalispoileralertonimarupuProjectileEntity(EntityType<? extends KondomnepitajzastoovoradimalispoileralertonimarupuProjectileEntity> type, LivingEntity entity, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public static KondomnepitajzastoovoradimalispoileralertonimarupuProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("entity.boat.paddle_water")), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}
	...
	public static KondomnepitajzastoovoradimalispoileralertonimarupuProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		...
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("entity.boat.paddle_water")), SoundSource.PLAYERS, 1, 1f / (RandomSource.create().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}
