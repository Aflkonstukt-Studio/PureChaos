...
public class Amogus3CrewEntity extends Monster {

	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(ChaosmodModEntities.AMOGUS_3_CREW.get(), 20, 4, 10));
	}

	private final ServerBossEvent bossInfo = new ServerBossEvent(this.getDisplayName(), ServerBossEvent.BossBarColor.GREEN,
			ServerBossEvent.BossBarOverlay.PROGRESS);

	public Amogus3CrewEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ChaosmodModEntities.AMOGUS_3_CREW.get(), world);
	}

	public Amogus3CrewEntity(EntityType<Amogus3CrewEntity> type, Level world) {
		super(type, world);
		xpReward = 99999;
		setNoAi(false);
	...
	public static void init() {
		SpawnPlacements.register(ChaosmodModEntities.AMOGUS_3_CREW.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getDifficulty() != Difficulty.PEACEFUL
						&& Monster.isDarkEnoughToSpawn(world, pos, random) && Mob.checkMobSpawnRules(entityType, world, reason, pos, random)));

		DungeonHooks.addDungeonMob(ChaosmodModEntities.AMOGUS_3_CREW.get(), 180);
	}
