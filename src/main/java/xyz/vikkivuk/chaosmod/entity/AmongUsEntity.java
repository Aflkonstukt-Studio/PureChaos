...
public class AmongUsEntity extends Monster {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(ChaosmodModEntities.AMONG_US.get(), 5, 1, 6));
	}
	...
	public AmongUsEntity(EntityType<AmongUsEntity> type, Level world) {
		...
		this.moveControl = new MoveControl(this) {
			@Override
			public void tick() {
				if (AmongUsEntity.this.isInWater())
					AmongUsEntity.this.setDeltaMovement(AmongUsEntity.this.getDeltaMovement().add(0, 0.005, 0));
				if (this.operation == MoveControl.Operation.MOVE_TO && !AmongUsEntity.this.getNavigation().isDone()) {
					double dx = this.wantedX - AmongUsEntity.this.getX();
					double dy = this.wantedY - AmongUsEntity.this.getY();
					double dz = this.wantedZ - AmongUsEntity.this.getZ();
					float f = (float) (Mth.atan2(dz, dx) * (double) (180 / Math.PI)) - 90;
					float f1 = (float) (this.speedModifier * AmongUsEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
					AmongUsEntity.this.setYRot(this.rotlerp(AmongUsEntity.this.getYRot(), f, 10));
					AmongUsEntity.this.yBodyRot = AmongUsEntity.this.getYRot();
					AmongUsEntity.this.yHeadRot = AmongUsEntity.this.getYRot();
					if (AmongUsEntity.this.isInWater()) {
						AmongUsEntity.this.setSpeed((float) AmongUsEntity.this.getAttribute(Attributes.MOVEMENT_SPEED).getValue());
						float f2 = -(float) (Mth.atan2(dy, (float) Math.sqrt(dx * dx + dz * dz)) * (180 / Math.PI));
						f2 = Mth.clamp(Mth.wrapDegrees(f2), -85, 85);
						AmongUsEntity.this.setXRot(this.rotlerp(AmongUsEntity.this.getXRot(), f2, 5));
						float f3 = Mth.cos(AmongUsEntity.this.getXRot() * (float) (Math.PI / 180.0));
						AmongUsEntity.this.setZza(f3 * f1);
						AmongUsEntity.this.setYya((float) (f1 * dy));
					} else {
						AmongUsEntity.this.setSpeed(f1 * 0.05F);
					}
				} else {
					AmongUsEntity.this.setSpeed(0);
					AmongUsEntity.this.setYya(0);
					AmongUsEntity.this.setZza(0);
				}
			}
		};
	}
