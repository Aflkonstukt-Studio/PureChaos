...
public class KondomnepitajzastoovoradimalispoileralertonimarupuProjectileEntity extends AbstractArrow implements ItemSupplier {
	...
	public KondomnepitajzastoovoradimalispoileralertonimarupuProjectileEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(PurechaosModEntities.KONDOMNEPITAJZASTOOVORADIMALISPOILERALERTONIMARUPU_PROJECTILE.get(), world);
	}
	...
	public static KondomnepitajzastoovoradimalispoileralertonimarupuProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		...
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.boat.paddle_water")), SoundSource.PLAYERS, 1, 1f / (RandomSource.create().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}
