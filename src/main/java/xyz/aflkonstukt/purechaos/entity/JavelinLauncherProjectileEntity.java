...
public class JavelinLauncherProjectileEntity extends AbstractArrow implements ItemSupplier {
	...
	public JavelinLauncherProjectileEntity(EntityType<? extends JavelinLauncherProjectileEntity> type, double x, double y, double z, Level world) {
		super(type, x, y, z, world, PROJECTILE_ITEM);
	}

	public JavelinLauncherProjectileEntity(EntityType<? extends JavelinLauncherProjectileEntity> type, LivingEntity entity, Level world) {
	...
	public static JavelinLauncherProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		JavelinLauncherProjectileEntity entityarrow = new JavelinLauncherProjectileEntity(PurechaosModEntities.JAVELIN_LAUNCHER_PROJECTILE.get(), entity, world);
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("purechaos:espuma")), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static JavelinLauncherProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		JavelinLauncherProjectileEntity entityarrow = new JavelinLauncherProjectileEntity(PurechaosModEntities.JAVELIN_LAUNCHER_PROJECTILE.get(), entity, entity.level());
		...
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("purechaos:espuma")), SoundSource.PLAYERS, 1, 1f / (RandomSource.create().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}
