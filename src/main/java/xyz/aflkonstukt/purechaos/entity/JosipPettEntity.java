...
public class JosipPettEntity extends TamableAnimal implements RangedAttackMob {
	public JosipPettEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(PurechaosModEntities.JOSIP_PETT.get(), world);
	}

	public JosipPettEntity(EntityType<JosipPettEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
		setCustomName(Component.literal("Evil Josip"));
		setCustomNameVisible(true);
		this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(PurechaosModItems.CORRUPTSTAFF.get()));
		this.setItemSlot(EquipmentSlot.OFFHAND, new ItemStack(PurechaosModItems.CORRUPTSTAFF.get()));
		this.setItemSlot(EquipmentSlot.HEAD, new ItemStack(PurechaosModItems.BEEF_ARMOR_HELMET.get()));
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(6, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = JosipPettEntity.this.getRandom();
				double dir_x = JosipPettEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = JosipPettEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = JosipPettEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	public void performRangedAttack(LivingEntity target, float flval) {
		KondomnepitajzastoovoradimalispoileralertonimarupuProjectileEntity.shoot(this, target);
	}

	@Override
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		JosipPettEntity retval = PurechaosModEntities.JOSIP_PETT.get().create(serverWorld);
	...
	public boolean isFood(ItemStack stack) {
		return Ingredient.of(new ItemStack(PurechaosModBlocks.GREEN_PLANT.get())).test(stack);
	}
