...
public class JukeBoxCaveFeature extends Feature<NoneFeatureConfiguration> {
	...
	public static Feature<?> feature() {
		...
		CONFIGURED_FEATURE = FeatureUtils.register("purechaos:juke_box_cave", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("purechaos:juke_box_cave", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("purechaos", "jukebox_cave"));
		if (template == null)
			return false;
