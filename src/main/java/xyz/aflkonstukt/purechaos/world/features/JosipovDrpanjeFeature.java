...
public class JosipovDrpanjeFeature extends Feature<NoneFeatureConfiguration> {
	...
	public static Feature<?> feature() {
		...
		CONFIGURED_FEATURE = FeatureUtils.register("purechaos:josipov_drpanje", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("purechaos:josipov_drpanje", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("purechaos", "josipspp"));
		if (template == null)
			return false;
