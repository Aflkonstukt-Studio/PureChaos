...
public class WoodRuinedPortalFeature extends Feature<NoneFeatureConfiguration> {
	...
	public static Feature<?> feature() {
		...
		CONFIGURED_FEATURE = FeatureUtils.register("purechaos:wood_ruined_portal", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("purechaos:wood_ruined_portal", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("purechaos", "wood_ruined_portal"));
		if (template == null)
			return false;
