...
public class BuffaloLakeFruitDecorator extends CocoaDecorator {
	public static Codec<BuffaloLakeFruitDecorator> CODEC = Codec.unit(BuffaloLakeFruitDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, new ResourceLocation("purechaos:buffalo_lake_tree_fruit_decorator"), () -> DECORATOR_TYPE);
	}

	public BuffaloLakeFruitDecorator() {
		super(0.2f);
	}
				...
				for (Direction direction : Direction.Plane.HORIZONTAL) {
					if (randomsource.nextFloat() <= 0.25F) {
						...
						if (context.isAir(blockpos)) {
							context.setBlock(blockpos, oriented(PurechaosModBlocks.EMEPEE.get().defaultBlockState(), direction1));
						}
					}
				}
			});
