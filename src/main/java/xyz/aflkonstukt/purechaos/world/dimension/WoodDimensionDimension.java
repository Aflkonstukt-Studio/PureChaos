...
public class WoodDimensionDimension {
	@Mod.EventBusSubscriber(bus = Mod.EventBusSubscriber.Bus.MOD)
	public static class Fixers {
		@SubscribeEvent
		public static void registerFillerBlocks(FMLCommonSetupEvent event) {
			Set<Block> replaceableBlocks = new HashSet<>();
			replaceableBlocks.add(Blocks.OAK_PLANKS);
			event.enqueueWork(() -> {
		...
		public static void registerDimensionSpecialEffects(FMLClientSetupEvent event) {
			DimensionSpecialEffects customEffect = new DimensionSpecialEffects(128, true, DimensionSpecialEffects.SkyType.NORMAL, false, false) {
				...
				public Vec3 getBrightnessDependentFogColor(Vec3 color, float sunHeight) {
					return new Vec3(0.839215686275, 0.780392156863, 0.082352941176);
				}
			...
			event.enqueueWork(() -> DimensionSpecialEffects.EFFECTS.put(new ResourceLocation("purechaos:wood_dimension"), customEffect));
		}
	}
}
